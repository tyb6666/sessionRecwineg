...
class SemanticIdTokenizer(nn.Module):
    ...
    @torch.no_grad
    @eval_mode
    def forward(self, batch: SeqBatch, negsam  = False) -> TokenizedSeqBatch:
        ...
        # clone ids 相关字段，避免修改原始 batch
        ids_clean = batch.ids.clone()
        ids_fut_clean = batch.ids_fut.clone()

        ids_clean[ids_clean >= flag_neg] -= flag_neg
        ids_fut_clean[ids_fut_clean >= flag_neg] -= flag_neg

        if self.cached_ids is None or ids_clean.max() >= self.cached_ids.shape[0]:
            B, N = ids_clean.shape
            ...
            sem_ids = self._tokenize_seq_batch_from_cached(ids_clean)
            seq_mask = batch.seq_mask.repeat_interleave(D, dim=1)
            sem_ids[~seq_mask] = -1

            sem_ids_fut = self._tokenize_seq_batch_from_cached(ids_fut_clean)

        # 修改负样本尾部 token 为 254
        neg_mask = batch.ids >= flag_neg
        if neg_mask.any():
            sem_ids = sem_ids.view(B, N, D)
            sem_ids[neg_mask, -1] = 254
            sem_ids = sem_ids.view(B, -1)
