...
def print_tensor_info(name, data):
    ...
    if isinstance(data, torch.Tensor):
        ...
        for key, value in data._asdict().items():
            if isinstance(value, torch.Tensor):
                ...
                print(f"{key}: 非 tensor 类型：{type(value)}")
    ...
    if isinstance(data, torch.Tensor):
        ...
        print(f"{name}: 不支持的类型内容，无法显示。")

    print()  # 空行分隔

def eval_model(model,eval_dataloader,accelerator,device,tokenizer,iter=1):
    metrics_accumulator = TopKAccumulator(ks=[1, 5, 10])
    model.eval()
    model.enable_generation = True
    with tqdm(eval_dataloader, desc=f'Eval', disable=not accelerator.is_main_process) as pbar_eval:
        for batch in pbar_eval:
            ...
            metrics_accumulator.accumulate_match3(actual=actual, top_k=top_k)
            metrics_accumulator.accumulate_valid_only(actual=actual, top_k=top_k)
    ...
    # output_dir = "./temp"
    # os.makedirs(output_dir, exist_ok=True)  # 如果没有就创建
...
def train(
    ...
    eval_model(model,eval_dataloader,accelerator,device,tokenizer)
    input()
    ...
    with tqdm(initial=start_iter, total=start_iter + iterations,
        ...
        for iter in range(iterations):
            ...
            for _ in range(gradient_accumulate_every):
                ...
                tokenized_data = tokenizer(data,negsam = True)
                # print_tensor_info("tokenized_data",tokenized_data)
                # input()
                ...
                if hasattr(model, "module"):
                    assert model.module.sem_id_embedder.emb.weight.grad is not None
                else:
            ...
            if (iter+1) % full_eval_every == 0:
                ...
                metrics_accumulator.reset()
                eval_model(model,eval_dataloader,accelerator,device,tokenizer, iter)
