...
class PreprocessingMixin:
    ...
    def _encode_text_feature(text_feat, model=None):
        if model is None:
            ...
            model_path = os.path.join(project_root, "models/sentence-t5-xl")
    ...
    def _df_to_tensor_dict(df, features):
        ...
        for feat in features:
            values = df.select(feat).to_series().to_list()
            out[feat] = pad_sequence(
                [torch.tensor(v, dtype=torch.long) for v in values],
                batch_first=True,
                padding_value=-1
            )
        ...
        # 处理 future 特征
        for feat in features:
            fut_values = df.select(feat + FUT_SUFFIX).to_series().to_list()
            out[feat + FUT_SUFFIX] = pad_sequence(
                [torch.tensor(v, dtype=torch.long) for v in fut_values],
                batch_first=True,
                padding_value=-1
            )

        # userId 是单个值，直接转换
        out["userId"] = torch.tensor(df.select("userId").to_series().to_list(), dtype=torch.long)
